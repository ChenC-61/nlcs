...
#' using factor parallel analysis and the original Velicer MAP criterion. An
#' N-LCS is constructed only when both methods recommend one factor.
#'
...
fit_nlcs <- function(hc_z, parallel_iter = 1000L, loading_tolerance = 1e-8) {
  ...
  class(model) <- "nlcs_model"
  raw_hc <- .raw_nlcs_metrics(x, model)
  model$cda_reference_median <- stats::median(raw_hc$cda_raw, na.rm = TRUE)
  model
}

#' @export
print.nlcs_model <- function(x, ...) {
